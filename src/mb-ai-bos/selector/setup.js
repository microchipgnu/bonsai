import { MbBosAiWallet } from './wallet';
...
const auth = async ({ logger, authWallet }) => {
  return {
    networkId() {
    ...
    async verifyOwner() {
      throw Error('authWallet:verifyOwner is deprecated');
    },
...
export function setupAuthWallet({ deprecated = false, networkId, signInContractId, relayerUrl }) {
  return async () => {
    ...
    const authWallet = new MbBosAiWallet({
      signInContractId,
      networkId,
      relayerUrl,
    });
